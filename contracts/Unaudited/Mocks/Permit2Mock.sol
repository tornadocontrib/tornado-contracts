...
import { IERC20 } from '../../Classic/interfaces/IERC20.sol';
...
contract Permit2Mock {
    ...
    function witness(PermitCommitments memory permitData) public pure returns (bytes32) {
        return keccak256(abi.encode(COMMITMENT_TYPEHASH, permitData.instance, permitData.commitmentsHash));
    }
    ...
    function depositPermit(bytes32[] memory _commitments, bytes memory permitData) external {
        ...
        if (sigType == SignatureType.PERMIT) {
            ...
            permit2.permitWitnessTransferFrom(
                ...
                owner,
                witness(PermitCommitments({ instance: address(this), commitmentsHash: commitmentsHash })),
                WITNESS_TYPE_STRING,
                signature
            );
        }
